...
public:
	...
	/// build the vegetable block, and append to list.
	void			init(const CVector &center, CVegetableClipBlock *vegetableClipBlock, CPatch *patch, uint ts, uint tt,
...
private:
	...
	uint8					_Ts, _Tt;
	/// The current distance type. 0 means all IG are created. NL3D_LANDSCAPE_VEGETABLE_BLOCK_NUMDIST means none.
	uint8					_CurDistType;


	/// The instance groups generated by this vegetable block, one for each distance type.
	CVegetableInstanceGroup	*_VegetableIG[NL3D_LANDSCAPE_VEGETABLE_BLOCK_NUMDIST];
