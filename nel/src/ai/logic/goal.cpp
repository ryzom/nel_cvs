...
namespace NLAILOGIC
{
	...
	IGoal::IGoal(const NLAIAGENT::IVarName &name, TTypeOfGoal mode) : IBaseBoolType()
	{
	...
	IGoal::IGoal(const NLAIAGENT::IVarName &name, std::list<const NLAIAGENT::IObjectIA *> &args, TTypeOfGoal mode)
	{
		_Name = (NLAIAGENT::IVarName *) name.clone();
		while ( !args.empty() )
		{
	...
	IGoal::IGoal(const IGoal &c) : IBaseBoolType()
	{
		if ( c._Name )
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	CGoal::CGoal(const NLAIAGENT::IVarName &name, TTypeOfGoal mode) : IGoal(name, mode)
	{
	}

	CGoal::CGoal(const NLAIAGENT::IVarName &name, std::list<const NLAIAGENT::IObjectIA *> &args, TTypeOfGoal mode) : IGoal(name, args, mode)
	{
	}

	CGoal::CGoal(const CGoal &c) : IGoal( c )
	{
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	CInternalGoal::CInternalGoal() : IGoal()
	{
		_Property = NULL;
	}
		
	CInternalGoal::CInternalGoal(const NLAIAGENT::IVarName &name, TTypeOfGoal mode) : IGoal(name, mode)
	{
		_Property = NULL;
	}

	CInternalGoal::CInternalGoal(const NLAIAGENT::IVarName &name, std::list<const NLAIAGENT::IObjectIA *> &args, TTypeOfGoal mode) : IGoal(name, args, mode)
	{
		_Property = NULL;
	}

	CInternalGoal::CInternalGoal(const CInternalGoal &c) : IGoal( c )
	{
		if ( c._Property != NULL )
			setProperty( c._Property );
		else
			_Property = NULL;
	}

	CInternalGoal::~CInternalGoal()
	{
		if ( _Property != NULL )
			_Property->release();
	}

	void CInternalGoal::operatorSuccess(NLAIAGENT::IBasicAgent *op)
	{
//		nlinfo("operatorSuccess: 0x%0x, %d, (0x%0x, %s)", this, _Successors.size(),op,(const char *)op->getType());
		std::vector<NLAIAGENT::IBasicAgent *>::iterator it_s = _Successors.begin();
		while ( it_s != _Successors.end() )
	...
	void CInternalGoal::operatorFailure(NLAIAGENT::IBasicAgent *op)
	{
		std::vector<NLAIAGENT::IBasicAgent *>::iterator it_s = _Successors.begin();
		while ( it_s != _Successors.end() )
		{
	...
	const NLAIC::IBasicType *CInternalGoal::clone() const
	{
		NLAIC::IBasicInterface *m = new CInternalGoal( *this );
		return m;
	}

	const NLAIC::IBasicType *CInternalGoal::newInstance() const
	{
		return clone();
	}


	void CInternalGoal::save(NLMISC::IStream &os)
	{			
	}

	void CInternalGoal::load(NLMISC::IStream &is)
	{
		IObjectIA::load( is );
	}

	void CInternalGoal::getDebugString(std::string &text) const
	{
		text += "<CInternalGoal> (";
		if ( _Name ) 
			text += _Name->getString();
	...
	bool CInternalGoal::isTrue() const
	{
		return false;
	}

	float CInternalGoal::truthValue() const
	{
		return 1.0;
	}

	const NLAIAGENT::IObjectIA::CProcessResult &CInternalGoal::run()
	{
		return IObjectIA::ProcessRun;
	}

	bool CInternalGoal::isEqual(const CInternalGoal &a) const
	{
		return false;
	}

	bool CInternalGoal::isEqual(const NLAIAGENT::IBasicObjectIA &a) const
	{
		return false;
	}

	const std::vector<NLAIAGENT::IObjectIA *> &CInternalGoal::getArgs()
	{
		return _Args;
	}

	const NLAIC::CIdentType &CInternalGoal::getType() const
	{
		return IdInternalGoal;
	}

	void CInternalGoal::setArgs(std::list<NLAIAGENT::IObjectIA *> &args)
	{
		std::list<NLAIAGENT::IObjectIA *>::iterator it_var = args.begin();
		while ( it_var != args.end() )
		{
	...
	const char *dbg_class_name = (const char *) getType();
		...
		if(className == NULL)
		{
			if( (*funcName) == constructor_name )
			{					
				NLAIAGENT::CObjectType *c = new NLAIAGENT::CObjectType( new NLAIC::CIdentType( CInternalGoal::IdInternalGoal ) );					
				r.push( NLAIAGENT::CIdMethod( 0 + IObjetOp::getMethodIndexSize(), 0.0, NULL, c) );					
			}
	...
	///\name Some IObjectIA method definition.
	//@{		
	NLAIAGENT::IObjectIA::CProcessResult CInternalGoal::runMethodeMember(sint32, sint32, NLAIAGENT::IObjectIA *)
	{
		return IObjectIA::CProcessResult();
	}

	NLAIAGENT::IObjectIA::CProcessResult CInternalGoal::runMethodeMember(sint32 index, NLAIAGENT::IObjectIA *p)
	{
		NLAIAGENT::IBaseGroupType *param = (NLAIAGENT::IBaseGroupType *)p;
	...
	sint32 CInternalGoal::getMethodIndexSize() const
	{
		return IBaseBoolType::getMethodIndexSize() + 3;
	}
	//@}

	void CInternalGoal::addSuccessor(NLAIAGENT::IBasicAgent *s)
	{		
		//nlinfo("addSuccessor: 0x%0x, %d, (0x%0x, %s)", this, _Successors.size(),s,(const char *)s->getType());
		_Successors.push_back(s);
		s->incRef();
	...
	void CInternalGoal::addPredecessor(NLAIAGENT::IBasicAgent *p)
	{
		_Predecessors.push_back(p);
		p->incRef();
	}

	bool CInternalGoal::operator==(const CInternalGoal &g)
	{
		if ( (*g._Name) == (*_Name) && _Args.size() == g._Args.size() )
			return true;
	...
	void CInternalGoal::setSender(NLAIAGENT::IBasicAgent *s)
	{
		_Sender = s;
	}

	void CInternalGoal::setReceiver(NLAIAGENT::IBasicAgent *r)
	{
		_Receiver = r;
	}

	NLAIAGENT::IBasicAgent *CInternalGoal::getSender()
	{
		return _Sender;
	}

	NLAIAGENT::IBasicAgent *CInternalGoal::getReceiver()
	{
		return _Receiver;
	}

	void CInternalGoal::cancel()
	{
		std::vector<NLAIAGENT::IBasicAgent *>::iterator i, end = _Successors.end();
		for ( i = _Successors.begin(); i != end; i++ )		
		{
	...
	float CInternalGoal::priority() const
	{
		if ( _Successors.empty() )
			return 0.0;
	...
	void CInternalGoal::setProperty(NLAIAGENT::IObjectIA *property)
	{
		if ( _Property != NULL )
		{
			_Property->release();	
