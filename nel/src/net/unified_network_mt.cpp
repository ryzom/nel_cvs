...
void	CUnifiedNetwork::release()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
	{
		...
		{
			for(uint j = 0 ; j < idAccess.value()[i].Connection.size (); j++)
			{
				if (idAccess.value()[i].Connection[j].CbClient->connected ())
					idAccess.value()[i].Connection[j].CbClient->disconnect();

				delete idAccess.value()[i].Connection[j].CbClient;
				idAccess.value()[i].Connection[j].CbClient = 0;
			}
			idAccess.value()[i].Connection.clear ();
		}
	}
...
void	CUnifiedNetwork::addService(const string &name, const CInetAddress &addr, bool sendId, bool external, uint16 sid, bool autoRetry)
{
	...
	addService (name, addrs, sendId, external, sid, autoRetry);
}

void	CUnifiedNetwork::addService(const string &name, const vector<CInetAddress> &addr, bool sendId, bool external, uint16 sid, bool autoRetry)
{
	...
	for (uint i = 0; i < addr.size(); i++)
	{
		...
		if (first && _TempDisconnectionTable.size() <= sid)
			_TempDisconnectionTable.resize(sid+1);

		try
		{
			cbc->connect(addr[i]);
			nlinfo("L5UNDBG: addService(name=%s, addr=%s): setAppId(sid=%d) (previous appId=%"NL_I64"x)", name.c_str(), addr[i].asString().c_str(), sid, cbc->getSockId()->appId());
...

		{
			...
			if (first && sid >= idAccess.value().size())
			...
			if(first)
			{
				...
				cnx = CUnifiedConnection(name, sid);
		...
		while (true)
		{
			...
			for (uint i = 0; i<connections.size(); ++i)
			{
				...
				if (cnx.EntryUsed && !cnx.IsServerConnection)
				{
					...
					if (cnx.IsConnected)
					{
						for (uint j = 0; j < cnx.Connection.size (); j++)
						{
							cnx.Connection[j].CbClient->update(0);
						}
					}
					else if (enableRetry)
					{
...
{
	{
		...
		for (i=0; i<_ConnectionRetriesStack.size(); ++i)
		{
			...
			try
			{
				for (uint j = 0; j < cnx.Connection.size (); j++)
				{
					cnx.Connection[j].CbClient->connect(cnx.ExtAddress[j]);
					nlinfo("L5UNDBG: updateConnectionTable(): in connection retry (cnx.ServiceName=%s, cnx.ServiceId=%d) setAppId(cnx.ServiceId=%d) (previous appId=%"NL_I64"x)", cnx.ServiceName.c_str(), cnx.ServiceId, cnx.ServiceId, cnx.Connection[j].CbClient->getSockId()->appId());
					cnx.Connection[j].CbClient->getSockId()->setAppId(cnx.ServiceId+L5_APPID_MARK(0x0203));
				}
		...
		for (i=0; i<_DisconnectionStack.size(); ++i)
		{
			...
			if (!cnx.IsExternal || !cnx.AutoRetry)
			{
				...
				if (!cnx.IsServerConnection)
				{
					...
					for (uint j = 0; j < cnx.Connection.size (); j++)
					{
						delete cnx.Connection[j].CbClient;
						cnx.Connection[j].CbClient = NULL;
					}
					cnx.Connection.clear ();
				}
		...
		for (i=0; i<_ConnectionStack.size(); ++i)
		{
			...
			else
			{
				...
				cnx.Connection.push_back (_ConnectionStack[i].SHost);
...
// ace todo can send on a selected connection
				if (connection.IsServerConnection)
					_CbServer->send(msgout, connection.Connection[0].HostId);
				else
					connection.Connection[0].CbClient->send(msgout);
		...
		for (i=0; i<connections.size(); ++i)
			if (connections[i].EntryUsed && connections[i].IsConnected)
				if (connections[i].IsServerConnection)
					_CbServer->send(msgout, connections[i].Connection[0].HostId);
				else
					connections[i].Connection[0].CbClient->send(msgout);
...
uint64 CUnifiedNetwork::getBytesSent ()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
		if (idAccess.value()[i].EntryUsed && !idAccess.value()[i].IsServerConnection)
			for (j=0; j<idAccess.value()[i].Connection.size (); ++j)
				sent += idAccess.value()[i].Connection[j].CbClient->getBytesSent();
...
uint64 CUnifiedNetwork::getBytesReceived ()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
		if (idAccess.value()[i].EntryUsed && !idAccess.value()[i].IsServerConnection)
			for (j=0; j<idAccess.value()[i].Connection.size (); ++j)
				received += idAccess.value()[i].Connection[j].CbClient->getBytesReceived();
...
uint64 CUnifiedNetwork::getSendQueueSize ()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
		if (idAccess.value()[i].EntryUsed && !idAccess.value()[i].IsServerConnection)
			for (j=0; j<idAccess.value()[i].Connection.size (); ++j)
				sent += idAccess.value()[i].Connection[j].CbClient->getSendQueueSize();
...
uint64 CUnifiedNetwork::getReceiveQueueSize ()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
		if (idAccess.value()[i].EntryUsed && !idAccess.value()[i].IsServerConnection)
			for (j=0; j<idAccess.value()[i].Connection.size (); ++j)
				received += idAccess.value()[i].Connection[j].CbClient->getReceiveQueueSize();
...
// ace todo can send on a selected connection
	if (cnx.IsServerConnection)
	{
		host = cnx.Connection[0].HostId;
		return _CbServer;
	}
	else
	{
		host = InvalidSockId;
		return cnx.Connection[0].CbClient;
	}
}
...
void	CUnifiedNetwork::autoCheck()
{
	...
	for (i=0; i<idAccess.value().size(); ++i)
	{
		...
		if(idAccess.value()[i].IsServerConnection)
		{
			nlassert (idAccess.value()[i].Connection.size()==1);
			nlassert (idAccess.value()[i].Connection[0].CbClient==0);
		}
		else
		{
			nlassert (idAccess.value()[i].Connection.size()>=1);
			for (j=0;j<idAccess.value()[i].Connection.size ();j++)
			{
				nlassert (idAccess.value()[i].Connection[j].HostId==0);
				nlassert (idAccess.value()[i].Connection[j].CbClient!=0);
			}
		}
		...
		uint64 appId = (idAccess.value()[i].IsServerConnection) ?
			(idAccess.value()[i].Connection[0].HostId->appId()) :
			(idAccess.value()[i].Connection[0].CbClient->getSockId()->appId());
