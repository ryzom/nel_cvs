...
//	return TRUE;
	...
	// Attention page en "dure" à enlever dès que tout est ok
	string url = "http://"+ConfigFile.getVar ("StartupHost").asString()+"/www_test/index.php";
...
void CNel_launcherDlg::OnTimer(UINT nIDEvent) 
{
	...
	if (patchState(state, log))
	{
		...
		CString strJava;
		strJava.Format("document.formPatch.listFile.value = '%s'",state);	
		COleVariant   varRet=ExecuteJavascript(strJava);
...
VARIANT CNel_launcherDlg::ExecuteScript(CString csCode, CString csLanguage)
{
	COleVariant   varRet;
	...
	if(lpDispatch)
	{
		...
		if(hr == S_OK)
		{
			...
			if(hr == S_OK)
			{
				...
				AfxMessageBox("accès à IHTMLWindow2 impossible");
...
VARIANT CNel_launcherDlg::ExecuteJavascript(CString csCode)
{
